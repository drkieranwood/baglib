function [ output_args ] = bag_load_nedrefer( input_args ,  plot_on )
%ROS_LOAD_NEDPTAM Loads the NED transformed reference data output by ROSbag files.
%   This script converts the csv file output from ROS into a MATLAB array
%   with field names, time-stamps. It is for a single reference pose
...
if ~isempty(temp)
    ...
    output_args = setfield(output_args,'tx',temp.data(:,2));               %Add the x position to the field 'tx'
    output_args = setfield(output_args,'ty',temp.data(:,3));               %Add the y position to the field 'ty'
    output_args = setfield(output_args,'tz',temp.data(:,4));               %Add the z position to the field 'tz'

    %Add the rotational data
    output_args = setfield(output_args,'rx',temp.data(:,5));               %Add the x(phi) rotation to the field 'rx'
    output_args = setfield(output_args,'ry',temp.data(:,6));               %Add the y(theta) rotation to the field 'ry'
    output_args = setfield(output_args,'rz',temp.data(:,7));               %Add the z(psi) rotation to the field 'rz'
    
    %This completes the data loading

    if plot_on
        h1 = figure('name','Ref: Time: Pos'); hold on;
        ...
        h2 = figure('name','Ref: Time: Rot'); hold on;
        temp_time = ( output_args.time - output_args.time(1) )/1000000000;
    ...
    warning('ROSLIB:LOAD_NEDREFER','Input file is empty');
    output_args = [];
end

end %ros_load_nedrefer
